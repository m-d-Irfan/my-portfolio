...
**Behaviour:** print all twenty-seven transitions as a numbered plain-text list — name, one-line summary, and the matching reference filename. Reuse the rows in `## Quick reference` above; do not invent new copy. No project access.

### transitions review — audit the project for fit

**Trigger phrases:** `transitions review`, "review my project", "audit my project for transitions", "where would transitions.dev help", "find places to use this skill".

> **Scope note.** This command answers *"what transition should I add here?"*. It does **not** tune values that already animate — that is `transitions audit` in the [`transitions-polish`](../transitions-polish/SKILL.md) skill. If the user's ask is about timing, easing, or duration consistency, hand off there.
...
**Trigger phrases:** `transitions refine`, "refine my transitions", "replace hardcoded durations with motion tokens", "audit my custom keyframes".

> **If `transitions-polish` is available, prefer `transitions audit` for this.**
> Both scan for ad-hoc motion values, but polish owns the values — it also
> checks open/close asymmetry, hover in/out, and stagger totals, which this
> command does not. Use `transitions refine` only when polish is not installed.
> This skill's own domain is *which transition to use*: `transitions apply`.
...
After installing any transition:

```bash
# 1. The token block is imported exactly once. Two copies means whichever
#    loads last silently wins, and edits to the other stop taking effect.
grep -rc "\-\-duration-fast" src/styles/ | grep -v ":0$"
# PASS: exactly one file, one occurrence

# 2. Every snippet kept its reduced-motion guard.
grep -c "prefers-reduced-motion" src/styles/transitions.css
# PASS: matches the number of transitions installed

# 3. No `transition: all` — snippets enumerate properties on purpose.
grep -rn "transition: all" src/
# PASS: no output

# 4. Only transform/opacity are animated. Animating width, height, top or
#    box-shadow triggers layout every frame.
grep -rnE "transition:[^;]*(width|height|top|left|box-shadow)" src/styles/
# PASS: no output, or a comment naming why layout animation is required
```

In a browser, per installed transition: trigger it twice in a row (a replay that
jumps means a missing reflow or an uncleaned `.is-closing`), then enable
reduced motion at the OS level and confirm it does not animate.

DevTools → Rendering → "Paint flashing" shows whether a transition is repainting
the whole region — if it is, it is animating a layout property.
...
This skill owns **which transition to install and how**. It does not own the
values it animates toward, or the states it animates between.

| Skill | Relationship |
|---|---|
| `transitions-polish` | Tunes motion that already animates against the token scale. Owns `transitions audit` and `transitions polish`; this skill owns `transitions reveal`, `transitions review` and `transitions apply`. Its `_root.css` is a subset of this skill's — install one, never both. |
| `ui-design-system` | Owns colour, type, spacing, radius and elevation tokens. A transition should reference `var(--color-*)` and `var(--radius-*)`, never a literal. Its `05-states.md` defines the loading / empty / error states that `14-skeleton-reveal` and `12-error-state-shake` animate between. |
| `forms-and-validation` | Owns form state and API error mapping. `12-error-state-shake` is the presentation layer for an error that skill produces; `13-input-clear-dissolve` and `10-success-check` sit on its submit state machine. |
| `performance-budget` | Owns the cost. Animate `transform` and `opacity` only — animating `width`, `height`, `top` or `box-shadow` triggers layout every frame. An entry animation on the LCP element delays LCP by exactly its duration, so keep reveals below the fold. |
| `admin-panel-builder` | Consumes these transitions in console views, and owns the density and depth values around them. |
| `impeccable` | Owns whether the motion is *good* — pacing, restraint, whether it earns its place. This skill owns whether it is correct. |
