...
Same scale the [transitions.dev](https://transitions.dev) Motion tokens tab exposes. Copy this skill's [`_root.css`](./_root.css) into your project once — **but only if `transitions-dev/_root.css` is not already imported.** This file is an exact subset of the token block at the top of that one; importing both duplicates the `:root` and whichever loads last silently wins. Once imported, reference any token as `var(--…)`. The `transitions-dev` transitions ship literal values, so they work without these tokens — install this skill to tune them against the shared scale.
...
The skill exposes two namespaced verbs. Every command starts with `transitions` so the invocation never collides with verbs from other skills installed in the same project. `audit` reports; `polish` applies.

> **Naming note.** This command is `transitions audit`, **not** `transitions review`. `transitions review` belongs to [`transitions-dev`](../transitions-dev/SKILL.md), where it suggests *whole transitions* to install. This skill only ever tunes values that already animate. If the user says "review my animations" and means timing/easing, run `transitions audit`; if they mean "what should I add here", hand off to `transitions-dev`.

### transitions audit — audit motion against the tokens

**Trigger phrases:** `transitions audit`, "review my animations", "audit the motion", "scan for ad-hoc transitions", "tokenize my animations", "where are my durations off", "align to the motion tokens", "make the timing consistent", "tune the durations / easing".

> **Ownership:** this command owns scanning for ad-hoc motion values.
> `transitions-dev`'s `transitions refine` overlaps it; prefer this one whenever
> polish is installed. Polish's own domain is values, not whole transitions —
> "which transition should this be" belongs to `transitions review` /
> `transitions apply`.
...
1. If the user has not run `transitions audit` in this session, run the same scan from **transitions audit** first (steps 1–4) to build the change list. If they named a specific file, line, or value, scope the scan to that context.
...
When this skill is installed in a project, the transitions.dev Refine panel's **Small refinement** tab automatically inlines the compact rules in [`_refine-rules.md`](./_refine-rules.md) into its agent prompt, so per-click suggestions honor these rules with zero extra file reads. `transitions audit` mirrors that audit at project scale; `transitions polish` writes the accepted tweaks to source.

## Verification

After a `transitions polish` pass:

```bash
# 1. No ad-hoc durations left outside the scale.
grep -rnE "(transition|animation)[^;]*[0-9]+m?s" src/ \
  | grep -vE "var\(--duration|0\.01ms"
# PASS: no output, or each hit is a documented exception

# 2. No easing outside the scale.
grep -rn "cubic-bezier(" src/ | grep -v "var(--ease"
# PASS: no output

# 3. The token block is imported exactly once — this skill's _root.css is a
#    SUBSET of transitions-dev's. Both imported means a duplicated :root.
grep -rc "\-\-duration-fast" src/styles/ | grep -v ":0$"
# PASS: exactly one file

# 4. Reduced motion still honoured after the pass.
grep -rn "prefers-reduced-motion" src/styles/
# PASS: at least one hit
```

Then the judgement checks a grep cannot make, in a browser:

- **Open and close feel different.** Close should be faster than open. If they
  are symmetric, the asymmetry rule was not applied.
- **Hover-in is faster than hover-out.** Reversed feels sluggish.
- **Nothing exceeds 400ms** except a deliberate page transition.
- **No entry animation on the LCP element** — its duration is added directly to
  LCP, which makes it a `performance-budget` defect, not a timing preference.

## Related skills

This skill owns **the values motion animates with**. It does not own which
transitions exist, what they animate toward, or what they cost.

| Skill | Relationship |
|---|---|
| `transitions-dev` | Owns the 27 transitions and their install. Its `_root.css` is a superset of this skill's — if it is already imported, these tokens are already defined and this skill's `_root.css` must be skipped. Commands: it owns `transitions reveal` / `review` / `apply`; this skill owns `transitions audit` / `polish`. |
| `ui-design-system` | Owns every non-motion token — colour, type, spacing, radius, elevation. A duration belongs here; a radius does not. Its `06-accessibility.md` owns the `prefers-reduced-motion` requirement this skill's values must survive. |
| `forms-and-validation` | Owns submit-state timing: the delay before a spinner appears, how long a success state holds, when an error shake replays. Tune those values here; the state machine lives there. |
| `performance-budget` | Owns cost. The polish rules and the budget agree: animate `transform` and `opacity`, never `width` / `height` / `top` / `box-shadow`. A duration on the LCP element is added directly to LCP, so an audit that finds a 600 ms hero reveal has found a performance defect, not a timing preference. |
| `admin-panel-builder` | Consumes the scale for console interactions — hover 150 ms, lift 200 ms, route change 250–300 ms, never over 400 ms. |
| `impeccable` | Owns whether the result *feels* right. This skill owns whether it is consistent with the scale. When the token-correct value still feels wrong, that is a judgement call and belongs there. |
